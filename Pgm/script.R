# Il vaut mieux supprimer dans la console et laisser le choix aux utilisateurs
# rm(list = ls())

# Très mauvaise pratique...
# setwd("/Home/work/Formation_Git_R_AM")

# Ne pas faire un install.packages pour ne pas intervenir
# sur les espaces des utilisateurs
...
# Modélisation ------------------------------------------------------------
# Il y a une fonction select dans le package MASS, qui fait que ça ne fonctionne pas
# il faut spécifier le package utilisé
df3 <- df %>%
  dyplr::select(surf, cs1, ur, couple, aged) %>%
  filter(surf != "Z")
